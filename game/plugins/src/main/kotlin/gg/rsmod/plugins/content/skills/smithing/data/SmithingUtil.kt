...
internal fun typeForChild(child: Int, bar: Bar) : String? = when {
    ...
    child == 32 -> when (bar) {
        ...
        Bar.ADAMANT -> "javelin heads"
        Bar.RUNE -> "javelin heads"
        else -> null
    }
    child == 33 -> when (bar) {
        Bar.BRONZE -> "javelin heads"
        Bar.IRON -> "javelin heads"
        Bar.STEEL -> "javelin heads"
        Bar.MITHRIL -> "javelin heads"
        else -> null
    }
    child == 34 -> when (bar) {
        Bar.BRONZE -> "bolts (unf)"
        Bar.IRON -> "bolts (unf)"
        Bar.STEEL -> "bolts (unf)"
        Bar.MITHRIL -> "bolts (unf)"
        Bar.ADAMANT -> "bolts(unf)"
        Bar.RUNE -> "ite bolts (unf)"
        else -> null
    }
    child == 35 -> when (bar) {
        Bar.BRONZE -> "limbs"
